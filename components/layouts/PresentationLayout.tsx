...
const SplitLayoutBlock = ({ slide, textWrapperClass }: { slide: SlideNode, textWrapperClass: string }) => {
  const mediaSrc = slide.splitSrc || slide.splitReverseSrc || undefined;
  const isLogo = !!mediaSrc && /logo|_icon\./i.test(mediaSrc);
  return (
    <>
      <div className="ambient-glow" />
      <div className={`split-layout-wrapper ${slide.splitReverseSrc ? 'split-layout--reversed' : ''}`}>
        <div className="split-layout">
          ...
          <div className={`split-layout__media gpu-accelerated ${isLogo ? 'split-layout__media--logo' : ''}`}>
            <DriftMedia intensity="medium" sequence="delayed" className="h-full">
              <img src={mediaSrc} alt="" style={{ height: '100%', width: '100%', objectFit: isLogo ? 'contain' : 'cover' }} />
            </DriftMedia>
          </div>
        </div>
      </div>
      ...
      {slides.map((slide, index) => {
        ...
        return (
          ...
          >
            ...
            <div style={{ width: '100%', zIndex: 10, position: 'relative', display: 'flex', justifyContent: 'center' }}>
              {(slide.splitSrc || slide.splitReverseSrc) ? (
                <SplitLayoutBlock slide={slide} textWrapperClass={textWrapperClass} />
              ) : (
                <StandardLayoutBlock slide={slide} textWrapperClass={textWrapperClass} hasBackground={hasBackground} />
              )}
            </div>
          </PresentationSlide>
        );
