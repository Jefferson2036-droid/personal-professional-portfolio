...
/**
 * Renders an absolute background image that shifts slightly based on its own
...
export function ParallaxBackground({ src, alt, mode = "standard", focal }: ParallaxBackgroundProps) {
  ...
  // y moves from -10% to +10% as the slide moves through the viewport
  ...
  // Calculate scroll velocity for the motion blur effect
  const scrollVelocity = useVelocity(scrollY);
  
  // Smooth the velocity so the blur doesn't jitter aggressively
  const smoothVelocity = useSpring(scrollVelocity, {
    damping: 50,
    stiffness: 400
  });

  // Transform velocity into a pixel blur value (0px to 10px)
  // We use absolute velocity so it blurs whether scrolling up or down
    ...
    >
      ...
      <motion.div
        ...
        style={{
          position: "absolute",
          inset: 0,
          background: "linear-gradient(180deg, rgba(10, 10, 10, 0) 0%, rgba(10, 10, 10, 0.14) 42%, rgba(10, 10, 10, 0.4) 100%)",
      ...
      <motion.div
        ...
        style={{
          position: "absolute",
          inset: 0,
          background: "radial-gradient(circle at center, rgba(0, 0, 0, 0) 36%, rgba(0, 0, 0, 0.16) 62%, rgba(0, 0, 0, 0.52) 100%)",
